...
const remoteClientVersion = "10.11.42";
...
export function startRemoteServer(
  ...
  app.get("/api/remote/alerts", fullControlOnly, (_req, res) =>
  ...
  app.post("/api/remote/alert", fullControlOnly, (req, res) => {
    const id = String(req.body?.id ?? "");
    const alert = alerts.list().find((item) => item.id === id);
    if (!alert) return res.status(404).json({ error: "No se encontró la alerta." });
    applyPatch({
      alert: {
  ...
  app.post("/api/remote/alert/hide", fullControlOnly, (_req, res) => {
    applyPatch({ alert: { visible: false } });
    return res.status(204).end();
  });
...
<html lang="es"><head>
  ...
  var alertsView=document.getElementById("alerts-view"),alertsList=document.getElementById("alerts-list"),openAlerts=document.getElementById("open-alerts"),hideAlert=document.getElementById("hide-alert");
  // Re-resolve the connection indicator before installing socket listeners.
  status=document.getElementById("status")||{textContent:"",classList:{add:function(){},remove:function(){}}};
  ...
  function showView(view){if((view==="songs"||view==="alerts")&&!fullControlEnabled)view="home";document.body.dataset.view=view;home.hidden=view!=="home";bibleView.hidden=view!=="bible";mediaView.hidden=view!=="media";songsView.hidden=view!=="songs";alertsView.hidden=view!=="alerts";if(view==="bible"&&!versions.length)loadVersions().catch(function(){versesElement.innerHTML="<div class=\"empty\">No se pudo cargar la Biblia.</div>";});if(view==="media")loadMeetings();if(view==="songs")loadSongMeetings();if(view==="alerts")loadAlerts();}
  ...
  function itemControls(item){if(Number(item.id)!==Number(activeMediaItemId))return "";if(item.kind==="presentation")return "<div class=\"item-controls\"><button data-presentation=\"-1\" aria-label=\"Diapositiva anterior\">← Anterior</button><button data-presentation=\"1\" aria-label=\"Diapositiva siguiente\">Siguiente →</button></div>";if(item.kind!=="video")return "";var video=projectionState&&projectionState.video||{};return "<div class=\"item-video-controls\"><div class=\"item-controls\"><button data-video=\"play\">"+(video.playing?"❚❚ Pausar":"▶ Reproducir")+"</button><button data-video=\"stop\">■ Detener</button><button data-video=\"mute\">"+(video.muted?"🔊 Activar sonido":"🔇 Silenciar")+"</button></div><label class=\"volume-label\">Volumen <input data-video=\"volume\" type=\"range\" min=\"0\" max=\"1\" step=\"0.05\" value=\""+(video.volume==null?1:video.volume)+"\"></label></div>";}
  ...
  function loadAlerts(){if(!fullControlEnabled)return Promise.resolve();alertsList.innerHTML="<div class=\"loading\">Cargando alertas…</div>";return request("/api/remote/alerts").then(function(items){if(!items.length){alertsList.innerHTML="<div class=\"empty\">No hay alertas guardadas en la computadora.</div>";return;}alertsList.innerHTML=items.map(function(item){var live=projectionState&&projectionState.alert&&projectionState.alert.visible&&projectionState.alert.message===item.message;return "<button type=\"button\" class=\"remote-alert"+(live?" live":"")+"\" data-alert=\""+escapeHtml(item.id)+"\"><i>!</i><span><b>"+escapeHtml(item.name||item.message)+"</b><small>"+escapeHtml(item.message)+"</small></span>"+(live?"<em>EN VIVO</em>":"")+"</button>";}).join("");}).catch(function(){alertsList.innerHTML="<div class=\"empty\">No se pudieron cargar las alertas.</div>";});}
  function setCapabilities(capabilities){fullControlEnabled=Boolean(capabilities&&capabilities.fullControlEnabled);openSongs.hidden=!fullControlEnabled;openAlerts.hidden=!fullControlEnabled;if(!fullControlEnabled&&(document.body.dataset.view==="songs"||document.body.dataset.view==="alerts")){showView("home");notify("El operador desactivó Control total");}if(!fullControlEnabled)songNav.hidden=true;}
