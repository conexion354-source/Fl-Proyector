...
<html lang="es"><head>
  ...
  <section id="songs-view" hidden><div class="view-toolbar"><button class="back" aria-label="Volver al inicio">← Volver</button><span class="view-toolbar-title">Canciones del culto</span><button id="clear-song" class="clear-live" aria-label="Quitar canción del aire" title="Quitar del aire"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="4" width="18" height="13" rx="2"/><path d="m8 9 8 8M16 9l-8 8M9 21h6"/></svg></button></div><h1>Canciones</h1><p class="sub">Elegí la reunión, la canción y la estrofa.</p><div class="media-picker"><select id="song-meeting"></select></div><section id="song-list" class="song-list"><div class="loading">Cargando canciones…</div></section><div id="song-nav" class="song-nav" hidden><button data-song-step="-1">← Anterior</button><div class="song-nav-status"><b id="song-nav-label">En vivo</b><span id="song-nav-title">Canción</span></div><button data-song-step="1">Siguiente →</button></div></section>
  ...
  function renderSongNav(){var song=songs.find(function(item){return item.active;});if(!song||!song.sections.length){songNav.hidden=true;return;}var index=Math.max(0,Number(song.activeSection)||0),section=song.sections[index]||song.sections[0],buttons=songNav.querySelectorAll("button");songNav.hidden=false;songNav.dataset.item=String(song.itemId);songNav.dataset.section=String(index);document.getElementById("song-nav-label").textContent=(section&&section.label||"Estrofa")+" · "+(index+1)+" de "+song.sections.length;document.getElementById("song-nav-title").textContent=song.title;buttons[0].disabled=index<=0;buttons[1].disabled=index>=song.sections.length-1;}
  function loadSongs(){var meetingId=songMeeting.value;if(!meetingId||!fullControlEnabled)return Promise.resolve();songList.innerHTML="<div class=\"loading\">Cargando canciones…</div>";return request("/api/remote/songs/"+encodeURIComponent(meetingId)).then(function(data){songs=data;if(!songs.length){songList.innerHTML="<div class=\"empty\">Esta reunión no tiene canciones.</div>";songNav.hidden=true;return;}songList.innerHTML=songs.map(function(song){return "<article class=\"song-card"+(song.active?" live":"")+"\" data-item=\""+song.itemId+"\"><button data-song-toggle=\"1\"><span><b>"+escapeHtml(song.title)+"</b><small>Tocá una estrofa para proyectarla · "+song.sections.length+" parte"+(song.sections.length===1?"":"s")+"</small></span><i class=\"badge\">"+(song.active?"EN VIVO":"CANCIÓN")+"</i></button><div class=\"song-sections\">"+song.sections.map(function(section){var active=song.active&&Number(song.activeSection)===Number(section.index);return "<button class=\"song-section"+(active?" active":"")+"\" data-song-section=\""+section.index+"\"><b>"+escapeHtml(section.label)+"</b><span>"+escapeHtml(section.text)+"</span></button>";}).join("")+"</div></article>";}).join("");renderSongNav();}).catch(function(error){songList.innerHTML="<div class=\"empty\">"+escapeHtml(error.message||"No se pudieron cargar las canciones.")+"</div>";songNav.hidden=true;});}
